...
class Sense():
    def __init__(self, picarx, camera = False, bus, time_delay=0.1):
        self.px = picarx
        self.bus = bus # sense_2_interprete_bus
        self.time_delay = time_delay
        self.ref = np.array(self.px.grayscale._reference)
    ...
    def get_grayscale_data(self):
        return np.array(self.px.grayscale.read()) - self.ref
    ...
    # Write captured image data to bus
    def capture_image_2_bus(self):
        while True:
            try:
                Vilib.take_photo(photo_name = self.image_name, path = self.path)
                self.bus.write(f'{self.path}/{self.image_name}')
                logging.debug("Image Captured")
            except:
                logging.debug("No Image")
            time.sleep(self.time_delay)
    
    # def camera_data_2_bus(self):
    #     while True:
    #         try:
    #             self.bus.write(self.capture_image())
    #         except:
    #             logging.debug("No camera data found")

    # Write grayscale data to bus
    def grayscale_data_2_bus(self):
        while True:
            try:
                self.bus.write(self.get_grayscale_data())
                logging.debug("Get grayscale data")

            except:
                logging.debug("No grayscale data found")
            time.sleep(self.time_delay)



class Interpret():
    def __init__(self, range = [0, 3600], polarity = False, bus, time_delay = 0.1):
