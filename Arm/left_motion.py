...
from left_perception import Perception

class Motion_Left():
    def __init__(self, perception):
        
        #coordinates for placing objects
        self.storage_coordinates = (12, 15, 7)
        ...
        while True:
            ...
            if self.perception.current_colour != "None" and self.perception.last_x < self.perception.img_size[0] // 2:
                ...
                if result:
                    ...
                    #move arm to drop position - storage location
                    result = self.arm_kinematics.setPitchRangeMoving((self.storage_coordinates[0], self.storage_coordinates[1], self.storage_coordinates[2]+5), -90, -90, 0)   
                    time.sleep(result[2]/self.sleep_divider)
                                    
                    block_rotation = getAngle(self.storage_coordinates[0], self.storage_coordinates[1], -90)
                    Board.setBusServoPulse(self.servo_2_id, block_rotation, self.gripper_closed)
                    time.sleep(self.sleep_time)

                    self.arm_kinematics.setPitchRangeMoving((self.storage_coordinates[0], self.storage_coordinates[1], self.storage_coordinates[2]+5), -90, -90, 0, 500)
                    time.sleep(self.sleep_time)
                                        
                    self.arm_kinematics.setPitchRangeMoving((self.storage_coordinates), -90, -90, 0, 1000)
                    time.sleep(self.sleep_time)

                    Board.setBusServoPulse(self.servo_1_id, self.gripper_closed - self.gripper_open, self.gripper_closed)
                    time.sleep(self.sleep_time)

                    self.arm_kinematics.setPitchRangeMoving((self.storage_coordinates[0], self.storage_coordinates[1], self.storage_coordinates[2]), -90, -90, 0, 800)
                    time.sleep(self.sleep_time)
